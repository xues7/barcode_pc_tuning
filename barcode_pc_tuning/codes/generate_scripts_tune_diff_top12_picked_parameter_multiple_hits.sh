...
do 
    ...
    do
        for f in /home/xues7/barcode_pc_tuning/bc4_data/${bp}_bp/*.gz;
        do
            for match in 6 
            do
                for mismatch in  -6 
                                ...
                                then
                                    ...
                                    do 
                                         ...
                                         do
                                             for diff12 in 6
                                             do 
                                                 echo bash /home/xues7/barcode_pc_tuning/codes/run_analysis_flexible_arguments_${mode}_diff12_multiple_hits.sh $match $mismatch $gap $extension $cutoff $length_cutoff `echo $(basename $f)|cut -d"." -f1` $bp $diff12> ./picked_parameter/run_`echo $(basename $f)|cut -d"." -f1`_match${match}_mismatch${mismatch}_gap${gap}_extension${extension}_mode${mode}_cutoff${cutoff}_length_cutoff${length_cutoff}_bp${bp}_diff12$diff12.sh
